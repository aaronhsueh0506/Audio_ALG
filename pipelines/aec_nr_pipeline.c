 ...
 * --delay-duty: sets AecConfig.delay_est_duty_cycle=1 — duty-cycles the AEC3
 *   matched-filter delay estimator once the estimate is solid (sampled-
 *   quality-verified ~zero cost; see aec.h for the exact schedule). Off by
 *   default (byte-identical to the reference).
 */
...
int main(int argc, char* argv[]) {
    ...
    float   *g_nr = NULL, *g_total = NULL, *g_aec = NULL, *extra = NULL;
    Complex *spec = NULL;
    ...
    if (!aec_only) {
        ...
        for (int k = 0; k < frame_sz; k++)
            synth_win[k] = sqrtf(0.5f * (1.0f - cosf(2.0f * (float)M_PI * k / frame_sz)));
    }
    ...
    if (!aec_only) {
        ...
        free(g_nr); free(g_total); free(g_aec); free(extra); free(spec);
        mmse_lsa_destroy(nr);
        fft_destroy(fft);
    }
    aec_destroy(aec);

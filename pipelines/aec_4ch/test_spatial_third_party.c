...
static float complex_error(kiss_fft_cpx a, kiss_fft_cpx b) {
...
static int test_phat_scalar_vs_dispatch(void) {
    ...
    kiss_fft_cpx x[N];
    kiss_fft_cpx y[N];
    kiss_fft_cpx scalar[N];
    kiss_fft_cpx dispatched[N];
    float worst = 0.0f;
    for (int i = 0; i < N; ++i) {
        x[i].r = random_signed();
        x[i].i = random_signed();
...
static int test_beamform_and_score_scalar_vs_dispatch(void) {
    enum { M = 4, F = 259 };
    kiss_fft_cpx w_storage[M][F];
    kiss_fft_cpx x_storage[M][F];
    const kiss_fft_cpx* w[M];
    const kiss_fft_cpx* x[M];
    kiss_fft_cpx beam_scalar[F];
    kiss_fft_cpx beam_dispatch[F];
...
static int test_srp_precompute_equivalence(void) {
    ...
    ArrayGeometry* geometry;
    SRP* srp_handle;
    kiss_fft_cpx* storage;
    kiss_fft_cpx* channels[4];
    float* golden;
    int golden_best = 0;
    ...
    storage = (kiss_fft_cpx*)malloc(
        (size_t)cfg.M * cfg.F * sizeof(kiss_fft_cpx));
...
static int reconstruct_matches(const GSC* g,
                               kiss_fft_cpx** x,
                               const kiss_fft_cpx* weights,
                               const kiss_fft_cpx* output) {
    for (int f = 0; f < g->F; ++f) {
        kiss_fft_cpx value = {0.0f, 0.0f};
        for (int m = 0; m < g->M; ++m) {
            kiss_fft_cpx w = weights[m * g->F + f];
            kiss_fft_cpx term;
...
static int test_gsc_weight_export(void) {
    ...
    kiss_fft_cpx* storage;
    kiss_fft_cpx* channels[4];
    kiss_fft_cpx output_original[65];
    kiss_fft_cpx output_exported[65];
    kiss_fft_cpx weights[4 * 65];
    ...
    storage = (kiss_fft_cpx*)malloc(4u * 65u * sizeof(kiss_fft_cpx));
 ...
 * via the aec_4ch wrapper's validate_config() -- a caller using GSC/gsc.h

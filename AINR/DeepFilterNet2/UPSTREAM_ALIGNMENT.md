# DeepFilterNet2 cascade/alpha fork — architecture and alignment record
...
| 3 | Noises mixed per example | `n_noises = rng.uniform(2,6)` → always 2–5, unconditional (`dataset.rs:1253-1254`) | **Aligned:** `min_noise_mix = 2`, `max_noise_mix = 5`; every required-noise sample uses that inclusive draw | Removes the former narrower, more stationary single-noise distribution. | **done** |
| 4 | Noise clipping, train split | `RandClipping::default_with_prob(0.1).with_c(0.01..0.5)` attached to each **noise lane** whenever `Split::Train` (`dataset.rs:705-709`) | 10% clipping exists, but is applied once to the **combined noise**; `generation_split=validation` disables it | Similar exposure but not distribution-identical: independent pre-combination lane clipping can differ from clipping the sum. | **remaining divergence; tune only with corpus A/B** |
| 5 | `RandRemoveDc` (p=0.25) + `RandLFilt` (p=0.25) | RemoveDC on speech; LFilt on speech and recorded-noise chains (`dataset.rs:644-651,693`) | **Aligned:** same probabilities, ordering and `[-3/8,3/8]` normalized second-order LFilt coefficients | Restores the lightweight spectral augmentation actually used upstream. | **done** |
| 6 | `p_biquad` | realised **0.0** (`DF_P_BIQUAD` never set; `dataset.rs:652,697`) | **Aligned:** example config now uses `0.0`; old three-section ±15 dB implementation remains available as an explicit opt-in | Removes up to ~45 dB of extra spectral tilt and its coupling to feature-normalizer calibration. | **done** |
| 6a | Generated full-band colored noise | each noise draw has 5% chance to use generated noise with `f_decay ∈ [-2,2]`, reaching native Nyquist (`dataset.rs:722-729,1180`) | **Aligned in distribution:** per-lane 5%, same decay range, native-rate FFT generation; local generator avoids upstream's repeated one-second tile | Covers high-frequency noise when recorded corpus lanes are bandwidth-limited. | **done** |
...
against autograd in `tests/test_dfn2_contract.py`).

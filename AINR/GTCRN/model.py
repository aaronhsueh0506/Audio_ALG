...
Adapted from gtcrn/gtcrn.py:
  - Removed einops dependency; shuffle() uses reshape instead of rearrange
  - Added erb_subband_1 / erb_subband_2 constructor params
  - DPGRNN width derived dynamically from encoder conv strides
"""
import numpy as np
import torch
...
class GTConvBlock(nn.Module):
    ...
    def shuffle(self, x1, x2):
        """x1, x2: (B, C, T, F) — channel shuffle without einops"""
        x = torch.stack([x1, x2], dim=1)         # (B, 2, C, T, F)
        x = x.transpose(1, 2).contiguous()        # (B, C, 2, T, F)
        x = x.reshape(x.shape[0], -1, x.shape[3], x.shape[4])  # (B, 2C, T, F)
        return x
...
class GRNN(nn.Module):
    ...
    def forward(self, x, h=None):
        """x: (B, seq_len, input_size); h: (num_layers, B, hidden_size)"""
        if h is None:
...
class GTCRN(nn.Module):
    def __init__(self, erb_subband_1=65, erb_subband_2=64, nfft=512, fs=16000):
        super().__init__()
        ...
        # Derive DPGRNN width from encoder conv strides
        # Input F = erb_subband_1 + erb_subband_2
        # Conv1: kernel=(1,5), stride=(1,2), padding=(0,2) → F_out = (F+4-5)//2+1 = (F-1)//2+1
        # Conv2: same params → F_out = (F_prev-1)//2+1
        f = erb_subband_1 + erb_subband_2
        f = (f - 1) // 2 + 1   # after Conv1 stride=2
        f = (f - 1) // 2 + 1   # after Conv2 stride=2
        self.dpgrnn1 = DPGRNN(16, f, 16)
        self.dpgrnn2 = DPGRNN(16, f, 16)
    ...
    def forward(self, spec):
        """
        spec: (B, F, T, 2)  — return_complex=False convention
        returns: enhanced spec (B, F, T, 2)
        """
        ...
        feat = self.erb.bm(feat)    # (B, 3, T, erb_subband_1+erb_subband_2)
        feat = self.sfe(feat)       # (B, 9, T, ...)

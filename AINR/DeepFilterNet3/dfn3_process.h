 ...
 *     網路: (feat_erb, feat_spec) -> (erb_mask, coefs)      <-- 只有這一段是學來的
 *     C   : erb_mask/coefs -> mask 展開 · deep filter · band split
 *           · post-filter · atten_lim · ISTFT + OLA -> frame
 *
 * ⚠ 網路只出 mask 與 coefficient，合成全部在這裡。compose() 是 parameter-free
 ...
 *   - band gain -> bin gain: erb_mask @ ERB_inv^T (mode=1)
 *   - deep filter: 最低 DF_BINS 個 bin 上 DF_ORDER-tap 複數 FIR
 *   - band split: DF 擁有 bin < DF_BINS，mask 擁有 bin >= DF_BINS (平行，非串聯)
 ...
 *    保留 (DFN3_SPEC_NORM_EPS)。這不是不一致，是刻意的:
...
#define DFN3_N_FFT          1024
#define DFN3_N_BINS         513   /* N_FFT/2 + 1 */
#define DFN3_WIN_LEN        1024  /* 分析窗長度 (= N_FFT) */
#define DFN3_HOP_LEN        512   /* 幀移 (= WIN_LEN/2, COLA) */
#define DFN3_OVL_LEN        (DFN3_WIN_LEN - DFN3_HOP_LEN)
...
#define DFN3_N_ERB          32
#define DFN3_DF_BINS        96    /* deep filter 擁有的 bin 數 (0..95) */
#define DFN3_DF_ORDER       5     /* FIR tap 數 */
#define DFN3_DF_LOOKAHEAD   1     /* 未來 tap 數 */
#define DFN3_MASK_LOOKAHEAD 1
...
#define DFN3_DF_HISTORY     (DFN3_DF_ORDER - DFN3_DF_LOOKAHEAD - 1)  /* 3 */
#define DFN3_DF_RING        DFN3_DF_ORDER                            /* 5 */

/* ---- 特徵正規化: = config.ini [feature] ----
 * ⚠ FEATURE_VERSION 必須與 train.py 逐字相同，否則 checkpoint 與這個前處理
 * 不是同一個合約。 */
#define DFN3_FEATURE_VERSION  "dfn2_libdf_wnorm_upstream_init_no_eps_v5"
...
#define DFN3_ERB_NORM_ALPHA         0.989f
#define DFN3_ERB_NORM_INIT_LO_DB  (-60.0f)   /* = libDF MEAN_NORM_INIT */
#define DFN3_ERB_NORM_INIT_HI_DB  (-90.0f)
#define DFN3_ERB_NORM_SCALE_DB      40.0f
#define DFN3_ERB_LOG_FLOOR          1e-10f   /* = libDF lib.rs:209 的字面值 */
...
#define DFN3_SPEC_NORM_ALPHA        0.989f
#define DFN3_SPEC_NORM_INIT_LO      0.001f   /* = libDF UNIT_NORM_INIT */
#define DFN3_SPEC_NORM_INIT_HI      0.0001f
#define DFN3_SPEC_NORM_EPS          1e-12f   /* ⚠ 見檔頭 parity 限制 (2) */
...
#define DFN3_ANALYSIS_SCALE   0.03125f       /* = 2*512/1024^2 * sqrt(1024) */

/* ---- 推論後處理: = config.ini [model] ---- */
#define DFN3_MASK_PF        0        /* Valin post-filter, 出貨 false */
#define DFN3_PF_BETA        0.02f    /* = 上游 PF_BETA */
...
typedef struct {
     ...
     * erb_fwd bin-major [DFN3_N_BINS][DFN3_N_ERB], erb_inv band-major
     * [DFN3_N_ERB][DFN3_N_BINS]; raw float32 from erb_fwd.bin/erb_inv.bin.
    ...
    float erb_norm_state[DFN3_N_ERB];

    /* 逐 bin 的 complex-magnitude EMA (只有 DF 那段 bin) */
    float spec_norm_state[DFN3_DF_BINS];

    /* deep filter 的複數 ring buffer: 未加 mask 的頻譜，最低 DF_BINS 個 bin。
     * ⚠ 存的是「未加 mask」的頻譜 — DFN3 是平行 band split，DF 作用在原始
    ...
    float hi_delay_re[DFN3_DF_RING][DFN3_N_BINS - DFN3_DF_BINS];
...
void dfn3_set_erb_matrices(DFN3State *st, const float *erb_fwd,
                             const float *erb_inv);
 ...
 * ⚠ 內部會套 DFN3_ANALYSIS_SCALE，呼叫端不要自己先乘。 */

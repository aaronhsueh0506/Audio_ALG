...
# Kept byte-for-byte compatible with checkpoints trained while this model still
# lived under DeepFilterNet2/.  The function name moved, the feature arithmetic
# did not.
...
#: autograd at 12 magnitudes; see tests/test_dfn3_contract.py.
#:
...
def dump_batch(dump_dir, noisy, clean, sr, enhanced=None):
    ...
    gradient hazard lives on the PREDICTION side: _SafeAngle amplifies most where
    the ENHANCED spectrum's magnitude is near 1e-5, so the prediction is what
    needs inspecting, not only the inputs.
    """
    ...
    # near_hazard counts samples inside a decade of _SafeAngle's worst magnitude
    # -- a time-domain proxy, but a lane with many of them is the one to open.
    lines = ['lane  clean_rms   noisy_rms   clean_peak  '
             'silent_target?  enh_rms     near_hazard%']
    lo, hi = SAFE_ANGLE_WORST_MAG / 3.0, SAFE_ANGLE_WORST_MAG * 3.0
    with torch.no_grad():
        for i in range(noisy.shape[0]):
            ...
            if enhanced is not None:
                ...
                mag = e.abs()
                pct = f'{100.0 * float(((mag > lo) & (mag < hi)).float().mean()):.2f}'
            else:
                e_rms, pct = '-', '-'
            lines.append(
...
def halt_on_non_finite(reason, *, model, optimizer, scheduler, noisy, clean,
...
def read_model_config(cfg):
    """Every DeepFilterNet3 constructor argument, defaults overlaid with [model].

    Defaults come from ``DeepFilterNet3.__init__``'s own signature rather than
    ...
    *and* inference.py, so a knob whose fallback was updated in one place built a
    ...
    kwargs = {
        name: param.default
        for name, param in inspect.signature(DeepFilterNet3.__init__).parameters.items()
    ...
    for name in cfg.options('model'):
        ...
        if kwarg not in kwargs:
            raise ValueError(
                f"[model] {name!r} is not a DeepFilterNet3 constructor argument "
...
def validate_signal_config(n_fft, win_len, hop_len, n_erb, df_bins, df_order,
    ...
    # DeepFilterNet3.__init__ -- every entry point constructs the model, so
...
def train(args):
    ...
    model = DeepFilterNet3(**model_cfg).to(device)
    ...
    print(f"DeepFilterNet3 training: SR={SR}, N_FFT={N_FFT}, WIN={WIN_LEN}, HOP={HOP_LEN}")
    ...
    if args.resume:
        ...
        # ⚠ The scheduler is deliberately NOT restored from the checkpoint.
        # load_state_dict would bring back CosineAnnealingLR's T_max from the run
        # that wrote it, overwriting the freshly-built
        # max(1, total_steps - warmup_steps).  Measured: a checkpoint from a
        # 100-epoch run stores T_max 4850; resuming it at epochs=120 builds 5850
        # and then restores 4850, giving a terminal lr of 1.02e-04 against
        # min_lr 1e-06 -- 102x -- plus a weight-decay discontinuity at the resume
        # step, because WD is re-derived per step while LR was not.  Neither
        # epochs nor batch_size is in the checkpoint contract, so nothing catches
        # the mismatch.
        # Upstream is stateless here (df/train.py:194-195, :268-269): it indexes
        # precomputed schedule arrays by step.  Fast-forwarding a fresh scheduler
        # reproduces that and stays correct across an epochs/batch/corpus change.
        ...
        print(f"  scheduler rebuilt for epochs={epochs} and fast-forwarded "
              f"{global_step} steps (lr={optimizer.param_groups[0]['lr']:.4e})")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        with tqdm.tqdm(train_loader, desc=f"Epoch {epoch}/{epochs}") as pbar:
            for batch_idx, (noisy, clean) in enumerate(pbar):
                ...
                spec_c, feat_erb, feat_spec, _ = extract_dfn3_features(
        ...
        # ⚠ Never overwrite dfn3_last.pth with a poisoned state.  The gradient
        ...
        if poisoned or optim_bad or not math.isfinite(val_loss):
            ...
            print(f'      {os.path.join(output_dir, "dfn3_last.pth")} left as-is')
            raise NonFiniteTraining(
        ...
        torch.save(ckpt, os.path.join(output_dir, 'dfn3_last.pth'))

        if is_best:
            best_val_loss = val_loss
            no_improve = 0
            torch.save(ckpt, os.path.join(output_dir, 'dfn3_best.pth'))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='DeepFilterNet3 Training')

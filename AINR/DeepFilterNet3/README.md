...
`dfn3_process.h/.c` owns the streaming STFT, ERB/complex normalization,
parameter-free low/high band composition, attenuation limit, and WOLA. The
accelerator only needs to emit `(erb_mask, coefs)`. Run `make -C .. test-simd`
for NEON/scalar digest parity or `make -C .. SIMD=0 test` for scalar only.

Use `dfn3_compose_stream()` at the accelerator boundary. Unlike DFN2, DFN3's
low DF branch reads the raw spectrum while the ERB mask owns only the high
branch, so mask/DF lookahead run in parallel. The shipped 1/1 setting therefore
has a one-hop streaming delay (10.67 ms at 48 kHz). The API returns the exact
output frame index and rejects a head before its lookahead input exists.

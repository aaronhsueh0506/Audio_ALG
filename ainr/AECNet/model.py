...
from dataset_gen.aec import frames_from_seconds  # noqa: E402
...
class AecNet(nn.Module):
    ...
    def reset_state(state: Optional[Dict], reset: Optional[torch.Tensor]) -> Optional[Dict]:
        ...
        Pair with ``dataset_gen.aec.lane_reset_mask`` on the batch's
    ...
    def forward_spec(self, y_spec: torch.Tensor, x_spec: torch.Tensor,
        ...
        ``(F, T)`` layout of ``dataset_gen.aec.stft`` never has to be transposed

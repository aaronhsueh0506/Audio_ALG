# DeepFilterNet2/ vs upstream DeepFilterNet3 — alignment record
...
alignment buys zero. All six trainers in `ainr/` sharing one seed *is* load-bearing:
it makes the train/val split identical across models, and in the AEC cascade a
stage-1 model split differently from the stage-2 model that consumes it would leak
stage-2's validation set into stage-1 training.

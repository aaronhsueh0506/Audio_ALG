...
⚠ Stage 1 regresses the ECHO ITSELF; its target is ``echo``.  That is why the
six stems are stored separately instead of as a (mic, clean) pair.  A model
trained to output S+N directly, or to emit a mask on the microphone, would be
doing AEC, residual suppression and noise reduction in one opaque step, and no
measurement afterwards could attribute a failure to any of them.

Everything a model may need is derivable from the stems:
    Y = mic_postclip     X = far_render     D = echo
    S = near_speech      N = local_noise
and nothing else is stored, because anything else would be a training decision
baked into the corpus.

REUSE
-----
The DSP primitives come from ``dataset_gen/dataset.py`` -- the NR generator and
...
class _seeded_global_rng:
    ...
    The reused helpers in ``dataset_gen/dataset.py`` draw from the ``random``
...
class RenderedSequence:
    stems: torch.Tensor              # (6, T) float32, channel order = STEM_ORDER
...
class AecSequenceRenderer:
    ...
    def _load_rir(self, path: str) -> torch.Tensor:
        ...
        if file_sr != self.sr:
            audio = torchaudio.functional.resample(audio, file_sr, self.sr)
        # Only the FULL RIR is used.  ``prepare_rir``'s late-suppressed target
        # exists for the NR generator's dereverberation target; here the near
        # talker's reverberation is desired signal the canceller must preserve,
        # and the echo's reverberation is exactly what it must remove.
        _target_rir, full_rir = prepare_rir(
            audio, self.sr,
    ...
    def _render_impl(self, plan: SequencePlan) -> RenderedSequence:
        ...
        near_raw = (fftconvolve(near_dry, self._load_rir(near_rir_path))
                    if has_near else torch.zeros(n_samples))
        ...
        if has_near and float(echo.abs().max()) > 0:
            ...
            near_speech = _scale_to_ratio(near_speech, echo, sr, ser_db)
        elif has_near:
            ser_db = float('inf')          # no echo: signal-to-echo is unbounded
            near_speech = _scale_to_active_dbfs(
                near_speech, sr,
        ...
        # ⚠ ONE common scale across ALL SIX stems.  Scaling only the mic would
        # break mic_preclip == S + N + D; scaling everything except the
        # reference would silently change the ERL the metadata claims.
        far_render, echo, near_speech, noise, mic_preclip = prevent_clipping(
            far_render, echo, near_speech, noise, mic_preclip,
        ...
        stems = torch.stack([far_render, echo, near_speech, noise,
...
def _scale_to_ratio(x: torch.Tensor, reference: torch.Tensor, sr: int,
    ...
    is the same definition ``dataset_gen/dataset.py`` uses for SNR.
    """

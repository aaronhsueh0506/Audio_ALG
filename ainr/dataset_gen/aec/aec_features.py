...
class AecGrid:
    ...
    def __post_init__(self):
        ...
        if self.win_len > self.n_fft:
            raise ValueError(
                f"win_len ({self.win_len}) > n_fft ({self.n_fft}): the window "
                f"would not fit in the transform"
            )
...
def stft(x: torch.Tensor, grid: AecGrid, center: bool = True) -> torch.Tensor:
    ...
    Leading dimensions are preserved, so a ``(B, 6, T)`` stem tensor transforms
...
class AecStems:
    ...
    def echo(self) -> torch.Tensor:
        """D: the echo actually present at the mic.  ⚠ Stage 1's target."""
    ...
    #
    # ⚠ There is deliberately no ``E`` or ``R`` accessor.  Both need D_hat,
    # which is a model output, not data.  Exposing them here would invite a
    # trainer to treat a residual as a supervision target, which is the one
    # thing this decomposition exists to prevent.
    @property
...
class SequenceChunkSampler(Sampler):
    ...
    def _build_schedule(self, epoch: int) -> List[List[int]]:
        ...
        if self.shuffle:
            ...
            # locality_preserving_random_split in dataset_gen/loader.py.

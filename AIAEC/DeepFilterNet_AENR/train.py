#!/usr/bin/env python3
"""DeepFilterNet-AENR training -- linear AEC -> joint RES+NR project variant.

用法:
    python3 train.py --config config.ini --packed-dir data_aec_48k/packed/all --gpu 0 --mmap
    python3 train.py --config config.ini --device cpu
    python3 train.py --config config.ini --resume output/dfn_aenr_e5.pth
    python3 train.py --config config.ini --resume output/dfn_aenr_e5.pth --reset-optimizer
    python3 train.py --config config.ini --seed 123

config.ini sections (see the shipped config.ini for every knob, documented):
    [signal]     sample rate + FFT grid. Must equal the grid
    ...
    [feature]    the SAME normalisation constants as
                 ...
                 config here that disagreed with DFN2's own tuned constants
                 would just be a second, drifting copy (see
                 AINR/DeepFilterNet2/UPSTREAM_ALIGNMENT.md for why those
                 particular numbers are not arbitrary)
    [model]      every DeepFilterNetAENR constructor keyword PLUS every
                 DeepFilterNet2 keyword it forwards via **kwargs (enc_ch,
                 emb_size, df_hidden, mask_pf, pf_beta, ...) -- both are
                 merged by read_model_kwargs's extra_bases; an unknown key
                 still raises
...
tensor shapes all match this model's (see ../README.md's DeepFilterNet-AENR
note); this script does not attempt that conversion.
...
from AINR.DeepFilterNet2.train import FEATURE_VERSION, read_feature_config
...
# merging them in from the base class as configurable would let a config
# pass them twice.
_DFN2_BASE_EXCLUDE = {'n_fft', 'sr', 'n_erb', 'df_bins', 'df_order',
...
def forward_batch(model, stems_batch, aec_grid, device, feature_cfg):
    ...
    spectral = build_spectral_model_view(
        view, aec_grid, dfn_model=model, dfn_feature_config=feature_cfg,
        dfn_feature_state=None,
    )
...
def main(args):
    ...
    feature_cfg = read_feature_config(cfg, aec_grid.sr, aec_grid.hop_len)
    model_kwargs = read_model_kwargs(
        cfg, DeepFilterNetAENR, extra_bases=(DeepFilterNet2,),
        exclude=_DFN2_BASE_EXCLUDE,
    )
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        torch.save(checkpoint, os.path.join(output_dir, 'dfn_aenr_last.pth'))
        if val_loss < best_val:
            best_val = val_loss
            no_improve = 0
            torch.save(checkpoint, os.path.join(output_dir, 'dfn_aenr_best.pth'))
        else:
            no_improve += 1

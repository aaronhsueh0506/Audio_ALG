...
Mirrors ``dataset_gen/gen_dataset.py``'s flags so the two generators are driven
...
if __package__ in (None, ''):
    # Allow `python3 gen_aec_dataset.py` from inside the directory, which is how
    # the NR generator is normally driven.
    _AIAEC = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    sys.path.insert(0, _AIAEC)
    sys.path.insert(0, os.path.join(os.path.dirname(_AIAEC), 'ainr'))
    __package__ = 'dataset_gen_aec'
...
WAV_ENCODINGS = {
    ...
    # mic_preclip == near_speech + local_noise + echo.  Quantising six stems

...
`.pt` shards. Sibling of `dataset_gen/` (the NR generator), which it reuses for
all shared DSP rather than forking it.
...
| 4 | `mic_preclip` | `S + N + D`, exactly, before any clipping or AGC. |
| 5 | `mic_postclip` | **Y** — what a model actually receives, after capture clipping/AGC. |
...
Y = S + N + D          microphone
X                      far-end reference
D_hat = AEC(Y, X)      ESTIMATED ECHO          <-- stage-1 output
E     = Y - D_hat      AEC output, by SUBTRACTION
R     = D - D_hat      residual echo — emerges, never a target
```

**⚠ Stage 1 regresses the echo itself. Its target is `echo`.** It is not
trained to output `S + N`, and it is not a mask on the microphone. A mask on Y
would make one network do AEC, residual suppression and noise reduction at
once, and no measurement afterwards could attribute a failure to any of them.
That is the entire reason the stems are stored separately instead of as a
`(mic, clean)` pair.
...
```python
from dataset_gen_aec import AecStems, PackedAecDataset
ds = PackedAecDataset('data_aec/packed/train')
v = ds.stems_of(0)
target = v.echo          # stage-1 target
mic    = v.mic_postclip  # stage-1 input, with v.far_render
```
...
**`ref_dropout` is load-bearing.** It supplies the idle-loss data and the hard
gate *"ref == 0 must imply output ≈ mic"*. During a dropout the far end is
genuinely silent — `X == 0` **and** `D == 0` — so the correct echo estimate is
zero and `E == Y` exactly. `[dropout] ref_dropout_echo_continues_p` can make
...
`dataset_gen/loader.py` splits the *generated* corpus — draw a permutation over
...
ship broken. `SequenceChunkSampler` exists to prevent that: each lane of a
batch walks one sequence in order, so batch *b+1* holds the next chunk of the
same sequence that batch *b* held, and state carries across. When a lane starts
a new sequence its `chunk_index` is 0 — that is the reset signal, and
`lane_reset_mask` reports it.

```python
from torch.utils.data import DataLoader
from dataset_gen_aec import PackedAecDataset, SequenceChunkSampler, aec_collate
...
| `gen_aec_dataset.py` | CLI — renders sequences to 6-channel WAV chunks |
...
python3 pack_aec_dataset.py --input data_aec/train --output data_aec/packed/train
python3 pack_aec_dataset.py --input data_aec/val   --output data_aec/packed/val
```
...
invariant is `mic_preclip == near_speech + local_noise + echo`. Quantising six
...
```bash
python3 -m pytest AIAEC/dataset_gen_aec/tests/ -q      # from ainr/
```
...
`dataset_gen/dataset.py`. Local noise uses the **same discrete SNR set** as the

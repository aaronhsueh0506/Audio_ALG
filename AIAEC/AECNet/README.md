...
Packed AEC shards from `AIAEC/dataset_gen_aec/pack_aec_dataset.py` — a directory of
...
and `lane_reset_mask` from `dataset_gen_aec`, and the train/val split, the
...
**⚠ Give `--val-packed-dir`.** `AIAEC/dataset_gen_aec/manifest.py` decides a

...
LOSS_VERSION = 'cagcrn_mse_sisnr_l1_v1'
...
def main(args):
    ...
    recipe = {
        ...
        'loss_mse_weight': cfg.getfloat('loss', 'mse_weight', fallback=1.0),
        'loss_si_snr_weight': cfg.getfloat(
            'loss', 'si_snr_weight', fallback=1.0),
        'loss_l1_weight': cfg.getfloat('loss', 'l1_weight', fallback=1.0),
        'loss_eps': cfg.getfloat('loss', 'eps', fallback=1e-8),
    }
    ...
    if recipe['schedule'] not in {'constant', 'warmup_cosine'}:
        raise ValueError(
            "CAGCRN scheduler must be 'warmup_cosine' or 'constant'")

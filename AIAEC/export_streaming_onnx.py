#!/usr/bin/env python3
"""Export AIAEC candidates with stateless accelerator state I/O.
...
an ordinary graph input/output. DeepFilterNet-AENR instead receives the
three-frame feature windows required by its lookahead input convolution.
...
from AIAEC.GTCRN_AENR.model import _dpgrnn_step, _gt_conv_block_step
...
from AINR.DeepFilterNet2.export_onnx import (
    ...
    StatelessDFN2Heads,
    _schema,
    initial_inputs as dfn_initial_inputs,
    set_onnx_metadata,
)
...
_C_BOUNDARIES = {
    ...
    'GTCRN_AENR': 'aiaec_process.c/aiaec_process.h',
    'DeepFilterNet_AENR': (
        'DeepFilterNet_AENR/dfn_aenr_process.c/.h'
    ),
}
...
_CONTROL_SEMANTICS = {
    ...
    'GTCRN_AENR': 'complex_mask_for_linear_error_spectrum',
    'DeepFilterNet_AENR': 'erb_mask_df_coefficients_alpha',
}
...
class StatelessOneFrameAIAEC(nn.Module):
    ...
    def forward(self, primary_ri: Tensor, far_end_ri: Tensor,
        ...
        elif self.model_name == 'GTCRN_AENR':
            learned = self._gtcrn(primary_ri, far_end_ri)
        else:
    ...
    def _gtcrn(self, primary: Tensor, far: Tensor) -> Tensor:
        model = self.model
        state = self.stream_state
        error = model.sfe(model.erb.bm(_mag_ri(primary)))
        reference = model.sfe(model.erb.bm(_mag_ri(far)))
        value = torch.cat((error, reference), dim=1)
        skips = []
        value = model.encoder.en_convs[0](value)
        skips.append(value)
        value = model.encoder.en_convs[1](value)
        skips.append(value)
        for index, block in enumerate(model.encoder.en_convs[2:], start=1):
            value = _gt_conv_block_step(
                block, value, state['enc_gt%d_depth' % index],
                state['enc_gt%d_tra' % index],
            )
            skips.append(value)
        value = _dpgrnn_step(
            model.dpgrnn1, value, state['dpgrnn1_inter_a'],
            state['dpgrnn1_inter_b'],
        )
        value = _dpgrnn_step(
            model.dpgrnn2, value, state['dpgrnn2_inter_a'],
            state['dpgrnn2_inter_b'],
        )
        for index, block in enumerate(model.decoder.de_convs[:3], start=1):
            value = _gt_conv_block_step(
                block, value + skips[5 - index],
                state['dec_gt%d_depth' % index],
                state['dec_gt%d_tra' % index],
            )
        value = model.decoder.de_convs[3](value + skips[1])
        mask_erb = model.decoder.de_convs[4](value + skips[0])
        mask = model.erb.bs(mask_erb)
        return mask.permute(0, 2, 3, 1)


class StatelessDFNAENRHeads(StatelessDFN2Heads):
    """DFN2 stateless heads with learned error/far conditioning in-graph."""
    ...
    def forward(
        ...
        feature_erb, feature_spec = self.model.condition_features(
            error_erb_window,
            error_spec_window,
            far_erb_window,
            far_spec_window,
...
class GraphSplit(NamedTuple):
    ...
    consumer takes them from here.  Re-deriving them downstream meant sniffing
    a feature-window shape or restating ``4 if DeepFilterNet_AENR else 2`` at
    five separate sites -- a family whose input arity changed would leave the
...
def _build(model_name: str, model):
    if model_name == 'DeepFilterNet_AENR':
        wrapper = StatelessDFNAENRHeads(model).eval()
        base = dfn_initial_inputs(model)
        inputs = (
            torch.randn(1, 1, DFN_INPUT_FRAMES, model.n_erb),
            torch.randn(1, 2, DFN_INPUT_FRAMES, model.df_bins),
            torch.randn(1, 1, DFN_INPUT_FRAMES, model.n_erb),
            torch.randn(1, 2, DFN_INPUT_FRAMES, model.df_bins),
        ) + base[2:]
        input_names = (
            'error_erb_window', 'error_spec_window',
            'far_erb_window', 'far_spec_window',
            'encoder_gru_hidden', 'erb_gru_hidden', 'df_gru_hidden',
            'df_convp_history',
        )
        output_names = (
            'erb_mask', 'df_coefs', 'df_alpha',
            'encoder_gru_hidden_next', 'erb_gru_hidden_next',
            'df_gru_hidden_next', 'df_convp_history_next',
        )
        split = GraphSplit(signal_inputs=4, head_outputs=3)
    ...
    primary = ('linear_error_ri' if model_name == 'GTCRN_AENR'
               else 'microphone_ri')
    input_names = (primary, 'far_end_ri') + wrapper.state_names
...
def main() -> None:
    ...
    metadata = {
        ...
        'input_feature_frames': (
            DFN_INPUT_FRAMES if args.model_name == 'DeepFilterNet_AENR' else 1
        ),

...
- paper-global soft delay distribution and weighted far-feature alignment;
...
hop rather than copied as 100. The default `alignment_mode="paper_global"`
emits one `D[B,dmax]` vector exactly as described. The explicitly different
`causal_running` mode emits `D[B,T,dmax]` using past evidence for packet
streaming.

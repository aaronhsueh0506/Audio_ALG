...
- **The checked-in VCTK benchmark tooling is non-functional as shipped.** `compute_improvement_vctk.py` / `regenerate_all_vctk.py` both import modules (`compute_improvement`, `regenerate_all`) that don't exist anywhere in the repo — confirmed `ModuleNotFoundError` on direct import. Both scripts also expect `noisy`/`clean` dataset subdirectories, while the actual downloaded dataset uses `noisy_testset_wav`/`clean_testset_wav`. No checked-in script combines VCTK iteration with `mode`/`strength` CLI selection. This session's 824-case VCTK re-runs used ad hoc scratchpad tooling, not anything in the repo — rebuilding a real, checked-in VCTK benchmark runner is a tracked follow-up.

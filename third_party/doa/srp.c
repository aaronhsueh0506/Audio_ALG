...
static int srp_init_pair_precompute(SRP* s)
{
    ...
    s->pair_steer = (kiss_fft_cpx***)calloc(s->num_angles, sizeof(kiss_fft_cpx**));
    if (!s->pair_steer) {
        srp_free_pair_precompute(s);
        return -1;
    }
    ...
    for (int a = 0; a < s->num_angles; a++) {
        s->pair_steer[a] = (kiss_fft_cpx**)calloc(s->num_pairs, sizeof(kiss_fft_cpx*));
        if (!s->pair_steer[a]) {
            srp_free_pair_precompute(s);
            return -1;
        }
        ...
        for (int pp = 0; pp < s->num_pairs; pp++) {
            s->pair_steer[a][pp] = (kiss_fft_cpx*)malloc(s->F * sizeof(kiss_fft_cpx));
            ...
            for (int f = 0; f < s->F; f++) {
                ...
                s->pair_steer[a][pp][f] = c_mul(c_conj(s->a_array[a][i][f]),
                                                s->a_array[a][j][f]);
            }
        }
    }
    ...
    s->pair_phat =
        (kiss_fft_cpx**)calloc(s->num_pairs, sizeof(kiss_fft_cpx*));
    if (!s->pair_phat) {
        srp_free_pair_precompute(s);
        return -1;
    }
    for (int pp = 0; pp < s->num_pairs; ++pp) {
        s->pair_phat[pp] =
            (kiss_fft_cpx*)malloc(s->F * sizeof(kiss_fft_cpx));
        if (!s->pair_phat[pp]) {
            srp_free_pair_precompute(s);
            return -1;
        }
...
void srp(SRP* s, kiss_fft_cpx** X, const int* mask)
{
    int M = s->M;
    int F = s->F;
    int A = s->num_angles;
    ...
    for (int f = s->f_start; f <= s->f_end; f++) {
        ...
        for (int a = 0; a < A; a++) {
            ...
            for (int i = 0; i < M; i++) {
                for (int j = 0; j < M; j++) {
                    kiss_fft_cpx Rij = c_mul(X[i][f], c_conj(X[j][f]));

                    float mag = c_abs(Rij) + 1e-8f;
                    Rij = c_div_real(Rij, mag);

                    kiss_fft_cpx w_i = s->a_array[a][i][f];
                    kiss_fft_cpx w_j = s->a_array[a][j][f];

                    kiss_fft_cpx tmp = c_mul(c_conj(w_i), Rij);
                    kiss_fft_cpx val = c_mul(tmp, w_j);

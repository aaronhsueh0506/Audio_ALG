                ...
                const GSC_Config* cfg)
{
    ...
    // allocate P (F,M,M)
    g->P = (kiss_fft_cpx***)calloc(F, sizeof(kiss_fft_cpx**));
    if (!g->P) {
        gsc_destroy(g);
        return NULL;
    }
    for (int f = 0; f < F; f++) {
        g->P[f] = (kiss_fft_cpx**)calloc(M, sizeof(kiss_fft_cpx*));
        if (!g->P[f]) {
            gsc_destroy(g);
            return NULL;
        }
        for (int i = 0; i < M; i++) {
            g->P[f][i] =
                (kiss_fft_cpx*)malloc(M * sizeof(kiss_fft_cpx));
            if (!g->P[f][i]) {
                gsc_destroy(g);
                return NULL;
            }
    ...
    // wa (M,F)
    g->wa = (kiss_fft_cpx**)calloc(M, sizeof(kiss_fft_cpx*));
    if (!g->wa) {
        gsc_destroy(g);
        return NULL;
    }
    for (int m = 0; m < M; m++) {
        g->wa[m] = (kiss_fft_cpx*)calloc(F, sizeof(kiss_fft_cpx));
        if (!g->wa[m]) {
            gsc_destroy(g);
            return NULL;
        }
...
#endif
        g->scratch = (kiss_fft_cpx*)calloc(count, sizeof(kiss_fft_cpx));
        if (!g->scratch) {
            gsc_destroy(g);
            return NULL;
        }
                              ...
                              kiss_fft_cpx* effective_weights)
{
    ...
    /* ---------- temp buffers ---------- */
    kiss_fft_cpx* das = g->scratch_das;
    kiss_fft_cpx* wu = g->scratch_wu;
    kiss_fft_cpx* gsc_spec = g->scratch_spec;
    kiss_fft_cpx (*u)[g->F] =
        (kiss_fft_cpx (*)[g->F])g->scratch_u;

#if !GSC_USE_PROJECTION_BLOCKING
    kiss_fft_cpx (*B)[g->M][g->M] =
        (kiss_fft_cpx (*)[g->M][g->M])g->scratch_b;
#endif

    /* ---------- DAS beamforming ---------- */
    spatial_conj_beamform(
        (const kiss_fft_cpx* const*)a,
        (const kiss_fft_cpx* const*)X,
    ...
    for (int f = 0; f < g->F; f++) {
        ...
        for (int m = 0; m < g->M; m++) {
            kiss_fft_cpx aproj = c_mul(a[m][f], proj);
            u[m][f] = c_sub(X[m][f], aproj);
        }
    }
    ...
    for (int f = 0; f < g->F; f++) {
        ...
        for (int i = 0; i < g->M; i++) {
            for (int j = 0; j < g->M; j++) {
                kiss_fft_cpx aa_ij = c_mul(a[i][f], c_conj(a[j][f]));
                aa_ij = c_div_real(aa_ij, denom);
    ...
    for (int m = 0; m < g->M; m++) {
        for (int f = 0; f < g->F; f++) {
            ...
            for (int n = 0; n < g->M; n++) {
                kiss_fft_cpx term = c_mul(c_conj(B[f][n][m]), X[n][f]);
                u[m][f] = c_add(u[m][f], term);
            }
        }
    }
    ...
    /* ---------- wu = conj(wa)^T u ---------- */
    {
        const kiss_fft_cpx* u_channels[g->M];
        for (int m = 0; m < g->M; ++m) u_channels[m] = u[m];
        spatial_conj_beamform(
            (const kiss_fft_cpx* const*)g->wa,
            u_channels, g->M, g->F, 1.0f, wu);
    }

    /* ---------- gsc = das - wu ---------- */
    for (int f = 0; f < g->F; f++) {
        gsc_spec[f] = c_sub(das[f], wu[f]);
    ...
    if (effective_weights) {
        for (int f = 0; f < g->F; ++f) {
            float denom = 0.0f;
            kiss_fft_cpx beta = {0.0f, 0.0f};
            for (int m = 0; m < g->M; ++m) {
                kiss_fft_cpx wa_h_a =
                    c_mul(c_conj(g->wa[m][f]), a[m][f]);
                beta = c_add(beta, wa_h_a);
                denom += a[m][f].r * a[m][f].r +
                         a[m][f].i * a[m][f].i;
            }
            if (denom < 1e-12f) denom = 1e-12f;
            beta = c_div_real(beta, denom);
            for (int m = 0; m < g->M; ++m) {
                kiss_fft_cpx a_h = c_conj(a[m][f]);
                kiss_fft_cpx weight =
                    c_add(c_div_real(a_h, (float)g->M),
                          c_mul(beta, a_h));
                weight = c_sub(weight, c_conj(g->wa[m][f]));
    ...
    if (do_adapt_this_frame) {
        ...
        for (int f = 0; f < g->F; f++) {
            ...
            for (int i = 0; i < g->M; i++) {
                ...
                for (int k = 0; k < g->M; k++) {
                    kiss_fft_cpx term = c_mul(g->P[f][i][k], u[k][f]);
                    pu[i] = c_add(pu[i], term);
                }
            }
            ...
            for (int i = 0; i < g->M; i++) {
                kiss_fft_cpx term = c_mul(c_conj(u[i][f]), pu[i]);
                upu_c = c_add(upu_c, term);
            }
            ...
            for (int j = 0; j < g->M; j++) {
                ...
                for (int k = 0; k < g->M; k++) {
                    kiss_fft_cpx term = c_mul(c_conj(u[k][f]), g->P[f][k][j]);
                    q[j] = c_add(q[j], term);
                }
            }
            ...
            for (int i = 0; i < g->M; i++) {
                for (int j = 0; j < g->M; j++) {
                    kiss_fft_cpx term = c_mul(gain[i], q[j]);
            ...
            for (int i = 0; i < g->M; i++) {
                for (int j = i + 1; j < g->M; j++) {
                    ...
                    g->P[f][i][j] = avg;
                    g->P[f][j][i] = c_conj(avg);
                }
                g->P[f][i][i].i = 0.0f;
            }

            /* ---------- wa update: wa = leak*wa + mu * gain * conj(gsc) ---------- */
            kiss_fft_cpx gsc_conj = c_conj(gsc_spec[f]);

            for (int m = 0; m < g->M; m++) {
                kiss_fft_cpx update = c_mul(gain[m], gsc_conj);
                ...
                g->wa[m][f] = c_add(c_scale(g->wa[m][f], GSC_WA_LEAK), update);
            }
        }
...
void gsc_reset(GSC* g)
{
    ...
    for (int m = 0; m < g->M; ++m) {
        memset(g->wa[m], 0, (size_t)g->F * sizeof(kiss_fft_cpx));
    }
    g->initialized = 0;
    g->first_doa_found = 0;
    g->first_doa_frame = -1;
